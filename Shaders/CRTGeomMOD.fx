...
//	ui_max = BUFFER_WIDTH;
//	ui_step = 1;
	ui_label = "输入画面X/Y分辨率(像素)";
	ui_tooltip = "屏幕框架缓存器画面的分辨率(0 = 自动/全屏)";
> = int2(0, 0);

uniform int resize_method <
	ui_type = "combo";
	ui_items = " 从左上角简单缩放 \0 中心锁定32:9 \0 中心锁定21:9 \0 中心锁定16:9 \0 中心锁定15:9 \0 中心锁定16:10 \0 中心锁定4:3 \0 中心锁定5:4 \0 中心锁定1:1 \0 中心锁定4:5 \0 中心锁定3:4 \0";
	ui_label = "游戏所用的画面缩放方法";
	ui_tooltip = "选择游戏如何根据分辨率来缩放画面(简单缩放与绝大部分老游戏匹配 / 中心锁定与绝大部分最近的游戏匹配)";
> = 0;

uniform bool ROTATED <
	ui_label = "旋转";
	ui_tooltip = "旋转画面(90° CCW)";
> = false;

uniform int inp_screen_ratio <
	ui_type = "combo";
	ui_items = " 自动检测 \0 32:9 Screen \0 21:9 Screen \0 16:9 Screen \0 15:9 Screen \0 16:10 Screen \0 4:3 Screen \0 5:4 Screen \0 1:1 Screen \0 4:5 Screen \0 3:4 Screen \0";
	ui_label = "模拟屏幕比例";
> = 0;

uniform int inp_game_ratio <
	ui_type = "combo";
	ui_items = " 自动检测 \0 32:9 Display \0 21:9 Display \0 16:9 Display \0 15:9 Display \0 16:10 Display \0 4:3 Display \0 5:4 Display \0 1:1 Display \0 4:5 Display \0 3:4 Display \0";
	ui_label = "模拟游戏比例";
> = 0;
...
uniform bool USE_BACKGROUND <
	ui_label = "背景";
	ui_tooltip = "在画面后面全屏显示图片";
> = false;

uniform bool USE_OFF_BEZEL <
	ui_label = "关闭凹槽";
	ui_tooltip = "当阴极射线管效果未激活时使用另一种边缘凹槽效果";
> = false;
...
uniform float3 bg_col <
	ui_type = "color";
	ui_label = "背景颜色";
	ui_tooltip = "显示器外的背景颜色";
	ui_category = "美术高级设置";
> = float3(0.0, 0.0, 0.0);

uniform bool CRT_EFFECT <
	ui_label = "阴极射线管效果";
	ui_tooltip = "开关阴极射线管效果";
> = true;
...
//	ui_max = BUFFER_WIDTH;
//	ui_step = 1.0f;
	ui_label = "模拟贴图X/Y分辨率(像素)";
	ui_tooltip = "游戏用于创建扫描效果的本地分辨率 (0.0 = 自动/默认)";
> = float2(0.0, 0.0);
...
uniform float2 buffer_offset <
	...
	ui_max = 5.0f;
	ui_step = 0.1f;
	ui_label = "相对于缓存的偏移(像素)";
	ui_tooltip = "当贴图需要准确对齐时允许进行微小偏移调整";
...
uniform bool VERTICAL_SCANLINES <
	ui_label = "垂直扫描线";
	ui_tooltip = "获得垂直扫描线而不用选择屏幕";
> = false;

uniform int aperture_type <
	ui_type = "combo";
	ui_items = " 模拟栅格 (绿/洋红) \0 点遮罩1x1贴图 \0 点遮罩2x2贴图 \0";
	ui_label = "栅格遮罩类型";
> = 0;
...
uniform bool USE_BEZEL <
	ui_label = "凹槽";
	ui_tooltip = "添加全局的全屏凹槽/覆盖";
> = false;

uniform bool USE_FRAME <
	ui_label = "屏幕框架";
	ui_tooltip = "在显示器上添加屏幕框架覆盖";
> = false;

uniform bool USE_OVERLAY <
	ui_label = "屏幕覆盖";
	ui_tooltip = "在显示区域上添加屏幕覆盖";
> = false;
...
uniform bool ACTIVATION_PIXEL_TEST <
	ui_label = "开启像素测试";
	ui_tooltip = "只有当两个像素测试匹配颜色时才能获得效果。(位置应由1920x1080分辨率的后台缓存定义)";
...
technique GeomModCRT <ui_label = "GeomMod阴极射线管(老式显示器)";>
{
	pass PixelTest
	{
