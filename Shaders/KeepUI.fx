...
uniform bool bKeepUIOcclude <
    ui_category = "选项";
    ui_label = "阻挡辅助";
    ui_tooltip = "如果你注意到泛光类似的着色器有奇怪的图形问题，请设置为1。启用后可能会导致SSDO的问题。";
...
uniform float fKeepUIOccludeMinAlpha <
    ui_type = "slider";
    ui_category = "选项";
    ui_label = "阻挡辅助Alpha阈值";
    ui_tooltip = "设置阻挡帮助的最小不透明度阈值。如果用户界面的不透明度低于该阈值，阻挡辅助将不被应用。有助于屏幕空间的照明和景深着色器。";
    ui_min = 0; ui_max = 1;
> = 0;
#endif
...
technique FFKeepUI <
    ui_tooltip = "把这个放在你的滤镜列表的顶部，将UI保存为贴图，以便用FFRestoreUI恢复。\n"
                 "要使用它，你必须也开启 \"FF恢复UI\"。(放在列表最后)\n"; ui_label="FF保留UI";
>
{
    pass
    {
...
technique FFRestoreUI <
    ui_tooltip = "把这个放在列表最后来恢复由FF保留UI的UI贴图。\n"
                 "要使用它，你必须也启用\"FF保留UI\"。\n";ui_label="FF恢复UI";
>
{
    pass
