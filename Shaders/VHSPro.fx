...
uniform float screenLinesNum <
	...
	ui_tooltip = "屏幕分辨率（单位: 行）。\n改变预处理定义中的screenLinesRes，使其具有与此相同的值。";
> = (float)BUFFER_HEIGHT;

uniform bool VHS_Bleed <
	ui_label = "出血[VHSPro]";
	ui_tooltip = "启用光束屏幕出血（使图像模糊不清）。";
> = true;

uniform int VHS_BleedMode <
	ui_type = "combo";
	ui_items = "三相\0老三相\0双相(慢)\0三相(RetroArch)\0双相(RetroArch)\0";
	ui_label = "出血模式[VHSPro]";
	ui_tooltip = "不同出血模式间切换";
> = 0;
...
uniform float vignetteSpeed <
	...
	ui_min = 0.0;
	ui_max = 5.0;
	ui_label = "暗角脉动速度[VHSPro]";
	ui_tooltip = "暗角脉动速度(设置为0使其静止)";
> = 1.0;
...
uniform bool VHS_YIQNoise <
	ui_label = "信号噪声[VHSPro]";
	ui_tooltip = "给YIQ信号添加噪音，造成粉色（或绿色）噪音。";
> = true;

uniform int signalNoiseType <
	ui_type = "combo";
	ui_items = "类型1\0类型2\0类型3\0";
	ui_label = "信号噪音类型[VHS Pro]";
> = 0;
...
uniform float signalNoisePower <
	...
	ui_min = 0.0;
	ui_max = 1.0;
	ui_label = "信号噪声强度[VHSPro]";
	ui_tooltip = "信号噪声的功率。较高的值会使它变成绿色，较低的值会使它变成更多的粉红色。";
> =  0.83;
...
uniform float lineNoiseSpeed <
	...
	ui_min = 0.0;
	ui_max = 10.0;
	ui_label = "线路噪声速度[VHSPro]";
	ui_tooltip = "线路噪声闪烁延迟速度";
> = 5.0;

uniform bool VHS_TapeNoise <
	ui_label = "卡带噪声[VHSPro]";
	ui_tooltip = "添加像VHS卡带的滚动噪声";
> = true;
...
uniform float tapeNoiseTH <
	...
	ui_min = 0.0;
	ui_max = 1.5;
	ui_label = "卡带噪声数量[VHSPro]";
	ui_tooltip = "画面的卡带噪声强度";
> = 0.63;
...
uniform float tapeNoiseAmount <
	...
	ui_min = 0.0;
	ui_max = 1.5;
	ui_label = "卡带噪声透明度[VHSPro]";
	ui_tooltip = "图像的卡带噪声数量";
> = 0.05;
...
uniform float tapeNoiseSpeed <
	...
	ui_min = -1.5;
	ui_max = 1.5;
	ui_label = "卡带噪声速度[VHSPro]";
	ui_tooltip = "卡带噪声的滚动速度";
> = 1.0;

uniform bool VHS_ScanLines <
	ui_label = "扫描线[VHSPro]";
	ui_tooltip = "开启电视/阴极射线管的扫描线";
> = false;
...
uniform bool VHS_LinesFloat	<
	ui_label = "线浮动[VHSPro]";
	ui_tooltip = "使得屏幕的线条向上或向下浮动。在低屏幕线的分辨率下效果最好。";
> = false;
...
uniform float linesFloatSpeed <
	...
	ui_min = -3.0;
	ui_max = 3.0;
	ui_label = "线浮动速度[VHSPro]";
	ui_tooltip = "浮动线的速度（和方向）。";
> = 1.0;

uniform bool VHS_Stretch <
	ui_label = "拉伸噪声[VHSPro]";
	ui_tooltip = "启用一个在图像上上下滚动的拉伸噪音，模拟VHS磁带的磁干扰。";
> = true;
...
uniform float jitterHAmount <
	...
	ui_min = 0.0;
	ui_max = 5.0;
	ui_label = "隔行扫描数量[VHSPro]";
	ui_tooltip = "隔行扫描强度";
> = 0.5;

uniform bool VHS_Jitter_V <
	ui_label = "抖动(Jitter) [VHSPro]";
	ui_tooltip = "添加垂直抖动噪声";
> = false;
...
uniform float signalAdjustY <
	...
	ui_min = -0.25;
	ui_max = 0.25;
	ui_label = "信号Y偏移[VHSPro]";
	ui_tooltip = "对信号的亮度(Luma)部分进行移位/调整。";
> = 0.0;
uniform float signalAdjustI <
	ui_type = "slider";
	ui_min = -0.25;
	ui_max = 0.25;
	ui_label = "信号I偏移[VHSPro]";
	ui_tooltip = "对信号的色度(Chroma)部分进行移位/调整。";
> = 0.0;
...
uniform float signalShiftY <
	...
	ui_min = -2.0;
	ui_max = 2.0;
	ui_label = "信号调整Y [VHSPro]";
	ui_tooltip = "调整信号的亮度部分";
> = 1.0;
...
uniform float signalShiftI <
	...
	ui_min = -2.0;
	ui_max = 2.0;
	ui_label = "信号调整I [VHSPro]";
	ui_tooltip = "调整信号的色度部分";
> = 1.0;
...
uniform float gammaCorection <
	...
	ui_min = 0.0;
	ui_max = 2.0;
	ui_label = "信号Gamma校正[VHSPro]";
	ui_tooltip = "Gamma校正图像";
> = 1.0;

uniform bool VHS_Feedback <
	ui_label = "荧光粉尾迹[VHSPro]";
	ui_tooltip = "启用老式阴极射线管显示器的荧光粉尾迹。";
> = false;
...
uniform float feedbackFade <
	...
	ui_min = 0.0;
	ui_max = 1.0;
	ui_label = "荧光粉淡出[VHSPro]";
	ui_tooltip = "荧光粉尾迹的淡出时间";
> = 0.82;
...
uniform float feedbackThresh <
	...
	ui_min = 0.0;
	ui_max = 1.0;
	ui_label = "输入裁切[VHSPro]";
	ui_tooltip = "裁切尾迹";
> = 0.1;

uniform float3 feedbackColor <
	ui_type = "color";
	ui_label = "荧光粉尾迹颜色[VHSPro]";
	ui_tooltip = "尾迹颜色";
> = float3(1.0,0.5,0.0);

uniform bool feedbackDebug <
	ui_type = "bool";
	ui_label = "Debug尾迹[VHS Pro]";
	ui_tooltip = "开启仅荧光粉尾迹可视化";
> = false;
...
uniform int VHS_Filter <
	...
	ui_tooltip = "对图像进行线性过滤，提高质量。在预处理器定义中定义VHSLINEARFILTER才能生效，这里只是作为提醒。";
> = 0.0;
...
technique VHS_Pro <ui_label="专业家用录像系统(VHS)";>
{
	pass VHSPro_First
	{
