...
uniform float TransmissionMultiplier<
	...
	ui_label = "强度";
	ui_category = "一般";
	ui_tooltip = "清除的总体强度，负值对应更多的清除，正值对应更少的清除。";
	ui_min = -1; ui_max = 1;
	ui_step = 0.001;
> = -0.125;
...
uniform float DepthMultiplier<
	...
	ui_label = "深度灵敏度";
	ui_category = "一般";
	ui_tooltip = "这个设置是用来调整有多少去除是基于深度的，\n或者如果设置了正值，它实际上会在场景中增加雾。0意味着它不受深度的影响。";
	ui_min = -1; ui_max = 1;
	ui_step = 0.001;
> = -0.075;
	...
	uniform float VarianceCutoff<
		ui_type = "slider";
		ui_label = "变化截止";
		ui_category = "可变帧率映射(Variable Rate Map)";
		ui_tooltip = "用这个来调整ReVeil停止寻找雾气的差异程度（雾气在调试视图中应该是绿色的）。";
		ui_min = 0.1; ui_max = 0.249;
		ui_step = 0.001;
	> = 0.15;
	
	uniform bool UseVRS<
		ui_label = "实验性可变帧率映射支持";
		ui_category = "可变帧率映射(Variable Rate Map)";
		ui_tooltip = "使用这一点可以让ReVeil使用可变率贴图（VariableRateShading必须被启用，它才能做出改变。）";
	> = false;
	
	uniform int Debug <
		ui_type = "combo";
		ui_items = "无\0转变映射\0可变帧率映射\0";
		ui_label = "Debug试图";
		ui_category = "Debug";
	> = 0;
#else
...
technique ReVeil_Top <ui_tooltip = "这要放在你想应用ReVeil的所有着色器上面。\n(不要担心它看起来什么都不做，它在这里做的事情在应用ReVeil-底部之前不会生效。）\n\n"
								  "Part of Insane Shaders\n"
								  "By: Lord of Lunacy";ui_label="ReVeil-顶部";>
{
#if REVEIL_COMPUTE == 1
...
technique ReVeil_Bottom <ui_tooptip = "这在你想应用ReVeil的着色器下面。";ui_label="ReVeil-底部";>
{
